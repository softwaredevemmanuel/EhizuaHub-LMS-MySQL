...
const StudentsProgress = () => {
    ...
    useEffect(() => {
        async function fetchStudents() {
            try {
                const response = await axios.get('http://localhost:5000/api/auth/all-students', {
      ...
      {content
        ...
        .map((subContent, subIndex) => (
          <div key={subIndex}>
            ...
            {/* Find the matching percentage based on subTopic */}
            {percentage.map((percentageItem, percentageIndex) => {
              if (percentageItem.subTopic === subContent.subTopic) {
                return (
                  <option key={percentageIndex} value={percentageItem.score}>
                    {percentageItem.score}%
                  </option>
                );
              }
              return null; // Return null for non-matching subTopics
            })}
          </div>
        ))}
    </div>
  ...
  {/* Move the percentage mapping outside of the uniqueMainTopics loop */}
  {percentage.map((percentageItem, index) => (
    <option key={index} value={percentageItem.score}>
      {percentageItem.subTopic} {percentageItem.score}%
    </option>
  ))}
</div>
