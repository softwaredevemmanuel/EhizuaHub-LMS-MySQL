...
function InstructorCreateContent() {
  ...
  const [instructor_authorization, setInstructorAuthorization] = useState('');
...
useEffect(() => {
  ...
  if (storedLoginData && storedLoginData.login && storedLoginData.token && storedLoginData.instructor_authorization) {
    ...
    setInstructorAuthorization(storedLoginData.instructor_authorization)
  ...
  useEffect(() => {
    async function fetchMainTopic() {
        ...
        try {
          let login = JSON.parse(localStorage.getItem('InstructorLogin'));

          const response = await axios.get('http://localhost:5000/api/instructor/maintopic', {
            headers: {
              course: login.course,
            },
          });
    ...
    async function fetchSubTopic() {
      try {
        if (main_topic !== '') { // Check if a topic is selected
          ...
          const response = await axios.get('http://localhost:5000/api/instructor/subtopic', {
            headers: {
              course: login.course,
...
// ........................ Create Content API ...................
  ...
  const createContent = () => {
    if (main_topic && content) {
      ...
      axios.post("http://localhost:5000/api/instructor/create-content", {
        authHeader: instructor_authorization,
        main_topic: main_topic,
        content: content,
        course : course,
        sub_topic : sub_topic
...
export default InstructorCreateContent;
